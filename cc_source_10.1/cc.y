...
%token BREAK CASE CONST CONTINUE DEFAULT DO ELSE ENUM EXTERN
...
cast_expr
	: unary_expr				{ $$ = $1; }
	| '(' type_specifier2 ')' 
	  cast_expr					{ if ( $2->type == VOID && $2->ptrVect == NULL )
...
struct_declaration
	...
	| type_specifier
	  identifier ':' CONSTANT ';'	
	  							{ $$ = NULL;
	  							  if ( $1->type != CHAR )
									yyerror("bit field type error!");
								  else if ( $4 > 8 || $4 <= 0 )
									yyerror("bit field size error!");
								  else
								  {
									$$ = oprNode(DATA_DECL, 1, makeList($2));
									$1->bitField   = ($4 < 8)? $4: 0;
									$1->isUnsigned = 1; 
									$$->opr.attr   = $1;
								  }
