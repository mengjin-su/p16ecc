...
static int sizer(node *np, int type);
	...
	if ( ap->ptrVect || ap->isFptr )
		...
		switch ( ap->type )
		{
			...
			case UNION:
				if ( ap->newData )
				{
					n = sizer((node*)ap->newData, ap->type);
					break;
				}
...
static int sizer(node *np, int atype)
{
	int n = 0, v;
	...
	if ( ap->type == STRUCT )
	{
		...
		for (int i = 0; i < LIST_LENGTH(np); i++)	// lines of declair
		{
			...
			for (int j = 0; j < LIST_LENGTH(pp); j++)
			{
				...
				offset += sizer(ppp->opr.attr, TOTAL_SIZE);
			}
		}
	}
	return offset;
