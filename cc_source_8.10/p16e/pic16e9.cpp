...
void PIC16E :: leftOpr(Item *ip0, Item *ip1, Item *ip2)
{
	...
	if ( bit_shift > 0 )
	{
		...
		for(int i = byte_shift; i < size0; i++, inst = _RLF)
		{
			...
			else if ( indf0 && (size0-byte_shift) > 1 )
				regFSR0->inc(byte_shift-size0+1, true);	//ASM_CODE(_ADDFSR, indf0, byte_shift-size0+1);
		}
		if ( bit_shift > 1 )
		{
...
void PIC16E :: leftOpr(Item *ip0, Item *ip1)
{
	...
	for(int i = 0; i < size0; i++)
	{
		...
		if ( (i+1) < size0 && indf1 )
			regFSR0->inc(1, true);	// ASM_CODE(_ADDFSR, indf1, 1);
	}
}
...
void PIC16E :: leftOprIndf1(Item *ip0, Item *ip1)
{
	...
	for(int i = 0; i < size0; i++, inst = _RLF)
	{
		...
		if ( indf0 && size0 > 1 )
		{
			if ( (i+1) < size0 ) regFSR0->inc(1, true);
			else				 regFSR0->inc(1 - size0, true);
		}
	}
...
void PIC16E :: rightOpr(Item *ip0, Item *ip1)
{
	...
	if ( indf1 && start_position > 1 )
		regFSR0->inc(start_position-1, true);
