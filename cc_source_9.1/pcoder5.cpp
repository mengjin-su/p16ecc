	...
	switch ( code )
	{
		...
		case '|': case '&': case '^':
			...
			if ( DEPTH() == (depth+2) )
			{
				...
				if ( ip0->isOperable() && ip0->attr->type != SBIT &&
					 ip1->isOperable() && ip1->attr->type != SBIT )
				{
					pnp = new Pnode(code);
					addPnode(&mainPcode, pnp);
					pnp->items[1] = ip0;
		...
		case LEFT_ASSIGN:	case RIGHT_ASSIGN:
			...
			if ( DEPTH() == (depth+2) )
			{
				...
				if ( (ip0->isOperable() && ip1->isOperable() && ip0->isWritable() 		  &&
					  ip0->attr->type != SBIT && ip1->attr->type != SBIT 		  		   ) ||
					 (ip0->attr->type == SBIT && ip1->type == CON_ITEM && ip1->val.i <= 1 &&
					  (code == XOR_ASSIGN || code == OR_ASSIGN || code == AND_ASSIGN)	   )  )
				{
