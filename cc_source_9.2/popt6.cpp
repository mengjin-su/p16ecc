...
/* this optimization case seems not neccesary.
	if ( ptype == '=' && p1 && p1->type == '=' &&								 // {'=', [*t, X]}
		 ip0->type == TEMP_ITEM && ip0->attr->ptrVect && ip1->type == ID_ITEM && // {'=', [Y, (t)]}
		 p1->items[0]->type == ID_ITEM && p1->items[1]->type == INDIR_ITEM && 
		 ip0->val.i == p1->items[1]->val.i )
	{
		ip1->type = PID_ITEM;
		ip1->updateAttr(p1->items[1]->attr);	p1->items[1]->attr = NULL;
		pnp->updateItem(0, p1->items[0]);		p1->items[0] = NULL;		delete p1;
		return true;
	}
*/
