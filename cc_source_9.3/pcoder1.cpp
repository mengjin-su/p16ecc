...
void Pcoder :: romDataInit(char *dname, attrib *attr, node *init_data, Dnode *dp, int addr)
{
	...
	if ( dimDepth(attr) > 0 && init_data->type == NODE_LIST )
	{
		...
		for(int i = 0; i < length; i++)
		{
			...
			else
			{
				pnp = new Pnode(P_FILL);
				pnp->items[0] = intItem(0);
				pnp->items[1] = intItem(sizer.size(ap, TOTAL_SIZE));
				addPnode(&constPcode, pnp);
			}
		}
		delAttr(ap);
	...
	if ( dimDepth(attr) > 0 )		// is an array
	{
		if ( dimDepth(attr) == 1 && init_data->type == NODE_STR )
		{
			...
			attrib *ap  = cloneAttr(attr);
			decDim(ap);
			int size = sizer.size(ap, ATTR_SIZE);
			for (int i = 0; i < length; i++)
			{
				pnp = new Pnode(P_FILL);
	...
	else
	{
		if ( init_data->type == NODE_LIST )
		{
			...
			int struct_length = sizer.memberCount(attr);

			for (int i = 0; i < struct_length; i++)
			{
				attrib *ap = sizer.memberAttr(attr, i);
				if ( i < list_length )
					romDataInit(dname, ap, LIST_NODE(init_data, i), NULL, 0);
				else
				{
					pnp = new Pnode(P_FILL);
					pnp->items[0] = intItem(0);
					pnp->items[1] = intItem(sizer.size(ap, ATTR_SIZE));
					addPnode(&constPcode, pnp);
				}
			}
		}
...
Item *Pcoder :: parseInitItem(Item *ip)
{
	if ( ip->type == TEMP_ITEM && mainPcode )
	{
		if ( mainPcode->type == '=' && sameTemp(ip, mainPcode->items[0]) )
		{
			delete ip;
			ip = mainPcode->items[1];
			mainPcode->items[1] = NULL;
		}
	}
	return ip;
}
