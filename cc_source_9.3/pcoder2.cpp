				...
				switch ( ip0->type )
				{
					...
					case ACC_ITEM:
						if ( ptrWeight(attr) > 0 )
						{
							...
							if ( dimDepth(ap) > 0 )
							{
								...
								ip1 = makeOffset(ip1, sizer.size(ap, SUBDIM_SIZE));
								decDim(ap);
							}
							else // input_dim <= 0
							{
								mem_type = reducePtr(ap);
								ip1 = makeOffset(ip1, sizer.size(ap, ATTR_SIZE));
							}
					...
					case DIR_ITEM:
						...
						if ( ptrWeight(ip0->attr) > 1 )
						{
							attrib *ap = cloneAttr(attr);
							reducePtr(ap);
							ip1 = makeOffset(ip1, sizer.size(ap, INDIR_SIZE));
...
bool Pcoder :: mergeConst(int opcode, Item *ip0, Item *ip1)
{
	if ( ip0->type == CON_ITEM && ip1->type == CON_ITEM )
	{
		...
		switch ( opcode )
		{
			case '+':
			case '-':
				n = (opcode == '+')? ip1->val.i : -ip1->val.i;
				if ( ap0->ptrVect && !ap1->ptrVect ) n *= ip0->stepSize();
				ip0->val.i += n;
				break;

			case '*':		ip0->val.i *= ip1->val.i;	break;
			case '/':		ip0->val.i /= ip1->val.i;	break;
			case '%':		ip0->val.i %= ip1->val.i;	break;
			case '&':		ip0->val.i &= ip1->val.i;	break;
			case '|':		ip0->val.i |= ip1->val.i;	break;
			case '^':		ip0->val.i ^= ip1->val.i;	break;
			case LEFT_OP:	ip0->val.i <<= ip1->val.i;	break;
			case RIGHT_OP:	ip0->val.i >>= ip1->val.i;	break;
