...
int	Item :: stepSize(void)
{
	...
	switch ( attr->type )
	{
		...
		case UNION:	if ( pointed )
						return sizer.size(attr, INDIR_SIZE);
		default:	return 0;
	}
}
...
int Item :: storSize(void)
{
	switch ( type )
	{
		case ACC_ITEM:
		case TEMP_ITEM:
			return attr->ptrVect? 2: sizer.size(attr, ATTR_SIZE);
...
int Item :: basicType(int *ptr_vect)
{
	int attr_type = attr? attr->type : 0;
	*ptr_vect = (attr && attr->ptrVect)? attr->ptrVect[0]: 0;
	switch ( type )
	{
		case STR_ITEM:
		case LBL_ITEM:
...
bool moveMatch(Item *ip0, Item *ip1)
{
	...
	else if ( ap0 && ap1 )
	{
				  ...
				  !(ap1->ptrVect || ap1->isFptr) )
		{
			...
			{
                if ( sizer.memberCount(ap0) != sizer.memberCount(ap1) )
					retVal = false;
				else
				{
					for (int i = 0; i < sizer.memberCount(ap0); i++)
					{
						attrib *a0 = sizer.memberAttr(ap0, i);
						attrib *a1 = sizer.memberAttr(ap1, i);
						if ( sizer.size(a0, TOTAL_SIZE) != sizer.size(a1, TOTAL_SIZE) )
							retVal = false;
					}
				}
			}
...
static int conAttr(Item *ip)
{
	return sizer.size(ip->attr->isNeg? true: false, ip->val.i);
}
