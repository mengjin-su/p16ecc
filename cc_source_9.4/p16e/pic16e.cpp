...
PIC16E :: PIC16E(char *out_file, Nlist *_nlist, Pcoder *_pcoder, bool asm_opt)
{
	memset(this, 0, sizeof(PIC16E));	// clean up the class data

	if( out_file ) fout = fopen(out_file, "w");
	if ( fout == NULL )	fout = stdout;
	asm16e = new P16E_ASM(fout, asm_opt);
	nlist  = _nlist;
	pcoder = _pcoder;
...
int PIC16E :: outputData(Dlink *dlink)
{
	...
	for (Dnode *dnode = dlink->dlist; dnode; dnode = dnode->next)
	{
		...
		sprintf(buf, "%s\t%d", _RS, sizer.size(attr, TOTAL_SIZE));
