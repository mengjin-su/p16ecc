	...
	switch ( op->oper )
	{
		...
		case FUNC_DECL:
			...
			if ( np && np->type == NODE_OPR && np->opr.nops > 1 )
			{
				...
				{	// scan function parameter list
					...
					int list_length = LIST_LENGTH(parp->id.dim);
					if ( list_length > 0 && parp->id.attr )
					{
						while ( list_length-- )
						{
							node *np = LIST_NODE(parp->id.dim, list_length);
							if ( np->con.value != 0 ) errPrint("null dim only!");
							insertPtr(parp->id.attr, parp->id.attr->dataBank);
						}
						delNode((node*)parp->id.dim);
						parp->id.dim = NULL;
					}
				}
			}
		...
		case ENUM:
			...
			if ( np && np->type == NODE_LIST )
			{
				...
				for (int i = 0; i < np->list.nops; i++, enum_v++)
				{
					...
					int type = sizer.size(enum_s != 0, enum_v);
			...
			if ( op->nops > 0 )
			{
				...
				for (int i = 0; i < np->list.nops; i++)
				{
					...
					if ( mem_addr >= 0 )
					{
						...
						mem_addr += sizer.size(dnp->opr.attr, TOTAL_SIZE);
					}
		...
		case TYPEDEF:
			...
			if ( ndp )
			{
				if ( ap->type == ENUM )
				{
					int neg_size = sizer.size(enumNeg < 0, enumNeg);
					int pos_size = sizer.size(enumNeg < 0, enumPos);
