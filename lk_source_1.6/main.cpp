...
int main(int argc, char *argv[], char *env[])
{
	...
	if ( start_parsing )
	{
		...
		p16link.removeExtraJKLines();
		
		// allocate memory for (code & data)
		do
		{
			if ( p16link.errorCount > 0 ) 	   break;
			if ( p16link.scanSegmentGroups() ) break;

		} while ( p16link.confirmSegmentMem() );
		...
		if ( p16link.errorCount == 0 )
		{
			...
			p16link.outputMap(output, dataMem);
			delete [] output;
//			dataMem->printSpace(NULL);
		}		
