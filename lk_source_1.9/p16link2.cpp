...
bool P16link :: confirmSegmentMem(void)
{
	...
	for (Segment *seg = codeSegGroup; seg; seg = seg->next)
	{
		...
		int obj_line = seg->lines->lineno;
		for (line_t *lp = seg->lines; lp; lp = lp->next, obj_line++)
		{
			int ltype  = lp->type;
			item_t *ip = lp->items;
			...
			switch ( ltype )
			{
				...
				case 'W':
					for (; ip; ip = ip->next)
					{
						if ( expValue(ip, seg, symbList, &v, addr, obj_line) )
							errorCount++;
						addr++;
					}
					break;
				...
				case 'K':
					...
					{
						item_t *ip1 = ip->next;
						char  *lbl0 = expValue(ip,  seg, symbList, &v0, addr, obj_line);
						char  *lbl1 = expValue(ip1, seg, symbList, &v1, addr, obj_line);
						if ( lbl0 ) errorCount++;
						if ( lbl1 ) errorCount++;
						if ( ip1 && !lbl0 && !lbl1 )
						{
