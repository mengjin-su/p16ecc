...
{
	HexWriter hexWriter(filename);

	for (Segment *seg = codeSegGroup; seg; seg = seg->next)
	{
		int addr = seg->memAddr & 0x7fff;
...
void P16link :: outputMap(char *filename)
{
	int usedMemSize, totalMemSize;

	MapWriter mapWriter(filename);
	...
	dataMem->printSpace(mapWriter.fout);
	...
	// output calling map ...
	if ( fcallMgr )	fcallMgr->outputCallPath(mapWriter.fout);
