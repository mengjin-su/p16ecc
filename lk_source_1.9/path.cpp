...
#define TOOL_NAME		"p16ecc"	// compiler tool dir.
#define IS_SLASH(c) 	((c) == '/' || (c) == '\\')
#define WIN_START(c)    ((c) == '=' || (c) == ';' )
#define LINUX_START(c)  (WIN_START(c) || (c) == ':')
...
Path :: Path(char *env[])
{
	...
	for (int i = 0; env[i]; i++)
	    if ( memcmp(env[i], "OS=Windows", 10) == 0 )
	        windows_env = true;
	...
	for (int i = 0; env[i]; i++)
	{
		...
		{
		    if ( windows_env )
		        while ( !WIN_START(*p) )   { p--; len++; }
		    else
			    while ( !LINUX_START(*p) ) { p--; len++; }
